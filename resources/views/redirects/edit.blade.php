...
<div class="sa-wrap sa-card">
    ...
    <form method="post" action="{{ route('sitemap.redirects.update', $redirect) }}">
        @csrf
        @method('PUT')
        <div class="sa-form-row">
            <label for="old_url">Old URL (path)</label>
            <input id="old_url" class="sa-input" name="old_url" required value="{{ old('old_url', $redirect->old_url) }}">
        </div>
        <div class="sa-form-row">
            <label for="new_url">New URL</label>
            <input id="new_url" class="sa-input" name="new_url" value="{{ old('new_url', $redirect->new_url) }}">
        </div>
        <div class="sa-form-row">
        ...
        <div class="sa-form-row">
            ...
            <textarea id="notes" class="sa-textarea" name="notes">{{ old('notes', $redirect->notes) }}</textarea>
        </div>
        <button type="submit" class="sa-btn sa-btn-primary">Update</button>
    </form>
</div>
